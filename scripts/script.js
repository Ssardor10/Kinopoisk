...
const hostValue = "3d1d7f1b-a435-4ef1-84bc-20071b7ac435";
...
function randMovies (num){
    return Math.trunc(Math.random() * num + 1);  // 0 - 250 || 0 - 45
}
...
async function renderSolo (id) {
    ...
    .then(res => {
        ...
        frame.forEach(item => {
            // console.log(item)
            frames +=  `<img src="${item.previewUrl}" alt="">`
        });
        ...
        <ul>
            ...
            <li class="solo__primier">Dunyo: </li>
            <li class="solo__rating">Rayting: ${solo.ratingKinopoisk}</li>
        ...
        <div class="solo__facts">
            <h2 class=" trend__tv-title">Qizaqarli faktlar</h2>
        </div>
